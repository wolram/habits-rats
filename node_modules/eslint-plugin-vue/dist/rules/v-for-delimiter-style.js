	...
	module.exports = {
		...
		create(context) {
			...
			return utils.defineTemplateBodyVisitor(context, { VForExpression(node) {
				...
				const delimiterToken = tokenStore.getTokenAfter(node.left.length > 0 ? node.left[node.left.length - 1] : tokenStore.getFirstToken(node), (token) => token.type !== "Punctuator");
