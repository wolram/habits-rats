	...
	function buildSuggest(define, emits, nameWithLoc, context) {
		...
		if (certainEmits.length > 0) {
			...
			return [{
				...
				fix(fixer) {
					if (last.type === "array") return fixer.insertTextAfter(last.node, `, '${nameWithLoc.name}'`);
					else if (last.type === "object") return fixer.insertTextAfter(last.node, `, '${nameWithLoc.name}': null`);
					else return null;
				}
			}];
		}
