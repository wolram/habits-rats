		...
		*/
		...
		const visitor = {
			...
			"ArrayExpression, ArrayPattern"(node) {
				const firstToken = tokenStore.getFirstToken(node);
				const rightToken = tokenStore.getTokenAfter(node.elements[node.elements.length - 1] || firstToken, isClosingBracketToken);
			...
			ImportDeclaration(node) {
				...
				if (namedSpecifiers.length > 0) {
					...
					const rightBrace = tokenStore.getTokenAfter(namedSpecifiers[namedSpecifiers.length - 1], isClosingBraceToken);
			...
			"ObjectExpression, ObjectPattern"(node) {
				const firstToken = tokenStore.getFirstToken(node);
				const rightToken = tokenStore.getTokenAfter(node.properties[node.properties.length - 1] || firstToken, isClosingBraceToken);
