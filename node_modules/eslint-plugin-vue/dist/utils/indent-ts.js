		...
		*/
		...
		return {
			...
			"TSConstructorType, TSFunctionType"(node) {
				...
				const rightParenToken = tokenStore.getTokenAfter(node.params[node.params.length - 1] || leftParenToken, isClosingParenToken);
			...
			TSIndexSignature(node) {
				...
				const rightBracketToken = tokenStore.getTokenAfter(node.parameters[node.parameters.length - 1] || leftBracketToken, isClosingBracketToken);
			...
			TSImportType(node) {
				...
				processNodeList(args, leftParenToken, tokenStore.getTokenAfter(args[args.length - 1], isClosingParenToken), 1);
				if (node.qualifier) {
			...
			Decorator(node) {
				...
				if (decorators[0] === node) if (range[0] === node.range[0]) setOffset(tokenStore.getTokenAfter(decorators[decorators.length - 1]), 0, atToken);
