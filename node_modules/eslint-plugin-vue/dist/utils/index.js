	...
	*/
	...
	module.exports = {
		...
		isEmptyValueDirective(node, context) {
			...
			if ((valueText[0] === "\"" || valueText[0] === "'") && valueText[0] === valueText[valueText.length - 1]) valueText = valueText.slice(1, -1);
	...
	*/
	function getVueObjectType(context, node) {
		...
		switch (parent.type) {
			...
			case "CallExpression":
				if (getVueComponentDefinitionType(node) != null && skipTSAsExpression(parent.arguments.slice(-1)[0]) === node) return "definition";
				break;
			case "NewExpression":
